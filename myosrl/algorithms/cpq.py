...
class CPQ(nn.Module):
    ...
    def __init__(self,
        ...
        print(self.q_thres,self.qc_thres)
...
class CPQTrainer:
    ...
    def train_one_step(self, observations, next_observations, actions, rewards,  # costs,
        ...
        # 模型生成的动作
        with torch.no_grad():
            # model_actions, _ = self.model._actor_forward(observations, deterministic=True)
            model_actions, _ = self.model.actor(observations, True, True)
        # 计算成本
        costs = self.model.compute_action_cost(actions, model_actions)
    ...
    def evaluate(self, eval_episodes: int = 1000, eval_dataset: list = None):
        ...
        self.model.train()
        return np.mean(episode_rets_array) / self.reward_scale, np.mean(episode_costs_array) / self.cost_scale, np.mean(
            episode_lens_array)
