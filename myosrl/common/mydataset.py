...
def process_bc_dataset(dataset: dict, cost_limit: float, gamma: float, bc_mode: str):
    ...
    done_idx = np.where((dataset["done"] == 1))[0]
...
def process_sequence_dataset(dataset: dict, cost_reverse: bool = False):
    ...
    Args:
        ...
        dataset (dict): A dictionary representing the dataset,
                        with keys "observations", "actions", "rewards", "costs", "terminals", and "timeouts", # done
...
class Mimic3_Vent_Dataset(IterableDataset):
    """
    ...
    def __init__(self,
        ...
        # add a flag column "is_init" to the dataset to indicate whether it is the initial state of a new episode.
        if self.state_init:
    ...
    def __prepare_sample(self, idx):
        ...
        # If the current 'done' is 1, then 'next_observations' equals 'observations'.
        if done == 1:
    ...
    # Defines the iteration method for the dataset. It will loop indefinitely, randomly selecting an index 'idx' of a sample (based on the 'sample_prob' probability distribution), and yield the prepared sample.
    def __iter__(self):
        while True:
