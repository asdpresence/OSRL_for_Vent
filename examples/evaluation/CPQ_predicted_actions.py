...
def generate_actions(cpq_model, states):
    with torch.no_grad():
        actions = cpq_model.actor(states, cpq_model.vae.decode(states))
    return actions
...
for step in trange(95002, desc="Predicting"):
    ...
    predicted_actions = generate_actions(cpq_model, observations)
...
output_file = 'cpq_predicted_actions_50000.pkl'
