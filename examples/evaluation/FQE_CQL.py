...
from myosrl.algorithms import CQL
...
def generate_actions(cql_model, states):
    with torch.no_grad():
        actions = cql_model.actor(states)
        # actions = bcql_model.actor(states, cql_model.vae.decode(states))
        return actions


def evaluate_strategy(cql_model, q_network, dataset):
    ...
    with torch.no_grad():
        for states, _, _, _ in dataloader:
            ...
            actions = generate_actions(cql_model, states)
...
evaluate_strategy(cql_model, q_network, dataset)
