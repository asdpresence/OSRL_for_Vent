...
# generate actions
def generate_actions(bcql_model, states):
    with torch.no_grad():
        actions = bcql_model.actor(states, bcql_model.vae.decode(states))
    return actions

# load evaluation configs
class EvalConfig:
    path: str = "/home/wyf/projects/OSRL_for_Vent/examples/vent/logs/BCQL/bcql/bcql"
...
bcql_model.to(eval_config.device)
...
for step in trange(95002, desc="Predicting"):
    ...
    predicted_actions = generate_actions(bcql_model, observations)
...
output_file = 'bcql_predicted_actions_50000.pkl'
